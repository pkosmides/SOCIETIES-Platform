var Client = {
		...
		on_group: function(iq) {
			Client.log("Group IQ result", iq);
			Client.show_text('Groups available:');
			Client.show_html('<ul>');
			$(iq).find('item').each(function () {
	            var jid = $(this).attr('jid');
	            var name = $(this).attr('name') || jid;

	            Client.show_html("<li id='" + jid + "'>" +
	                            "<div class='group-contact offline'>" +
	                            "<div class='group-name'>" +
	                            name +
	                            "</div><div class='group-jid'>" +
	                            jid +
	                            "</div></div></li>");
		...
		on_group_error: function(iq) {
			Client.log("Group IQ error", iq);
			Client.show_text('No group available:');
		},
		...
		start: function() {
			...
			Client.log("Disco com");
			var NS_DISCO_ITEMS = 'http://jabber.org/protocol/disco#items';
			var iq = $iq({type: 'get', to: 'comunitizer.socialblend.local'}).c('query', {xmlns: NS_DISCO_ITEMS});
			Client.connection.sendIQ(iq, Client.on_group, Client.on_group_error);
		}
}
